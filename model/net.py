...
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '/media/user/HDD/ssx_project/pvtdiff/CamoDiffusion_v4/denoising-diffusion-pytorch')))
...
class OverlapPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=7, stride=4, in_chans=3, embed_dim=768, mask_chans=0):
        ...
        self.mask_proj = nn.Conv2d(1, embed_dim, kernel_size=patch_size, stride=stride,
                                       padding=(patch_size[0] // 2, patch_size[1] // 2))
...
class Decoder(Module):
    def __init__(self, dims, dim, class_num=2, mask_chans=1):
        ...
        self.time_embed2 = nn.Sequential(
            nn.Linear(2*self.time_embed_dim, 4* self.time_embed_dim),
            nn.SiLU(),
            nn.Linear(4 * self.time_embed_dim, 4*self.time_embed_dim),
        )
        self.time_embed3 = nn.Sequential(
            nn.Linear(4*self.time_embed_dim, 8 * self.time_embed_dim),
            nn.SiLU(),
        ...
        self.down_2x_2 = nn.Sequential(
            ...
            resnet_block(4*embedding_dim, 4*embedding_dim, time_emb_dim=4*self.time_embed_dim),
            ConvModule(embedding_dim*4, embedding_dim*4, kernel_size=3, padding=1, norm_cfg=dict(type='BN'))
        )


        self.down_2x_3 = nn.Sequential(
            ConvModule(embedding_dim*4, embedding_dim*8, kernel_size=3, stride=2, padding=1, norm_cfg=dict(type='BN')),  
            resnet_block(8*embedding_dim, 8*embedding_dim, time_emb_dim=8*self.time_embed_dim),
            ConvModule(embedding_dim*8, embedding_dim*8, kernel_size=3, padding=1, norm_cfg=dict(type='BN'))  
        )
    ...
    def forward(self, inputs, timesteps, x):
        ...
        c_fusion = self.final_conv(inputs)
        x = self.fft(x)
        x = torch.cat([c_fusion, x], dim=1)
        x = self.cat_conv(x)
...
class net(nn.Module):
    def __init__(self, class_num=2, mask_chans=0, **kwargs):
        ...
        self.spatial_fusion_stages = nn.ModuleList([
            spatial_RGBD_fusion(channels=64),    
            spatial_RGBD_fusion(channels=128),   
            spatial_RGBD_fusion(channels=320),   
            spatial_RGBD_fusion(channels=512)    
        ])
    ...
    def forward(self, x, timesteps, cond_img, depth_map):
        ...
        for i, (rgb_feat, depth_feat) in enumerate(zip(rgb_features, depth_features)):
            ...
            fused = self.spatial_fusion_stages[i](rgb_feat, depth_feat)
    ...
    def _init_weights(self):
        pretrained_dict = torch.load('/media/user/HDD/ssx_project/pvtdiff/CamoDiffusion_v4/pretrained_weights/pvt_v2_b4_m.pth') #for save mem
...
class GroupedChannelAttention(nn.Module):
    def __init__(self, in_planes, groups=4, ratio=16):
        super().__init__()
        ...
        self.mlps = nn.ModuleList([
            nn.Sequential(
                nn.Conv2d(self.group_planes, self.group_planes//ratio, 1),
                nn.ReLU(),
                nn.Conv2d(self.group_planes//ratio, self.group_planes, 1)
    ...
    def forward(self, x):
        ...
        for group_idx in range(self.groups):
            ...
            group_feat = x_groups[group_idx]
            max_pool = F.adaptive_max_pool2d(group_feat, 1)
            avg_pool = F.adaptive_avg_pool2d(group_feat, 1)
            
           
            max_attn = self.mlps[group_idx](max_pool)
            avg_attn = self.mlps[group_idx](avg_pool)
            group_attn = self.sigmoid(max_attn + avg_attn)
            
            attn_maps.append(group_attn)
        
      
        return torch.cat(attn_maps, dim=1)  # [B,C,1,1]
...
class MultiScaleSpatialAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        branch_outs = [conv(combined) for conv in self.convs]
        summed_attn = branch_outs[0] + branch_outs[1] + branch_outs[2]
     
        return summed_attn



class spatial_RGBD_fusion(nn.Module):
    def __init__(self, channels):
        super().__init__()
        self.fusion_channel_attention = GroupedChannelAttention(channels)
        self.fusion_spatial_attention = MultiScaleSpatialAttention()
        ...
        self.final_conv = nn.Conv2d(2*channels, channels, kernel_size=3, padding=1, bias=False)
    def forward(self,r,d):
        assert r.shape == d.shape,"rgb and depth should have same size"
        multiply = r*d
        spatialmap = self.fusion_spatial_attention(multiply)
        r_multiply = r*spatialmap
        d_multiply = d*spatialmap
        fusion1 = self.fusion_conv1(torch.cat([r_multiply, d_multiply], dim=1))

        concate = self.cat_conv(torch.cat([r, d], dim=1))
        concatemap = self.fusion_channel_attention(concate)
        r_cat = r*concatemap
        d_cat= d*concatemap
        fusion2 = self.fusion_conv2(torch.cat([r_cat, d_cat], dim=1))

        addfusion = fusion1+fusion2
        multiplyfusion = fusion1*fusion2
        fusion = self.final_conv(torch.cat([addfusion, multiplyfusion], dim=1))
...
class ProgressiveFusion(nn.Module):
    def __init__(self):
        ...
        # Stage1: 64x88x88 → 128x44x44 
        self.stage1 = FusionBlock(in_ch_low=64, in_ch_high=128, out_ch=128)
        
        # Stage2: 128x44x44 → 320x22x22 
        self.stage2 = FusionBlock(in_ch_low=128, in_ch_high=320, out_ch=320)
        
        # Stage3: 320x22x22 → 512x11x11 
        self.stage3 = FusionBlock(in_ch_low=320, in_ch_high=512, out_ch=512)
        
    def forward(self, features):
        f1, f2, f3, f4 = features
        
        # Stage1:  → 128x44x44
        fused_44 = self.stage1(f1, f2)
        
        # Stage2:  → 320x22x22
        fused_22 = self.stage2(fused_44, f3)
        
        # Stage3: → 512x11x11
        output = self.stage3(fused_22, f4)
...
class FusionBlock(nn.Module):
    ...
    def __init__(self, in_ch_low, in_ch_high, out_ch):
        ...
        self.process_high = nn.Sequential(
            nn.Conv2d(in_ch_low, out_ch, 3, stride=2, padding=1),  
    ...
    def forward(self, feat_high, feat_low):
        ...
        combined = torch.cat([addfusion, multipfusion], dim=1) 
