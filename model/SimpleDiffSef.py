...
class CondGaussianDiffusion(GaussianDiffusion):
    ...
    def forward(self, img, cond_img, depth_map, seg=None, extra_cond=None, *args, **kwargs):
        b, channels, h, w = img.shape
        cond_channels = cond_img.shape[1]
        cond_channels_depth = depth_map.shape[1] 
        assert channels == self.channels
        assert h == w == self.image_size
        assert cond_channels == self.cond_channels
        assert cond_channels_depth == 1
        ...
        return self.p_losses(img, times, cond_img, depth_map, seg, extra_cond, *args, **kwargs)

    def p_losses(self, x_start, times, cond_img, depth_map, seg=None, extra_cond=None, noise=None, *args, **kwargs):
        ...
        if seg is not None:
            ...
            x, log_snr = self.q_sample(x_start=x_start, times=times, noise=noise) # 返回的是加噪图
        
        # predict and take gradient step
        time_start = time.time()
        model_out = self.model(torch.cat([x, extra_cond], dim=1), log_snr, cond_img, depth_map)
    ...
    @torch.no_grad()
    def sample(self, cond_img, depth_map, extra_cond=None, verbose=True):
    ...
    @torch.no_grad()
    def p_sample_loop(self, shape, cond_img, depth_map, extra_cond, verbose=True):
        self.history = []
        img = torch.randn(shape, device=self.device)
        conditioning_features, depth_features= self.model.extract_features(cond_img, depth_map)
        ...
        for i in tqdm(range(self.num_sample_steps), desc='sampling loop time step', total=self.num_sample_steps,
            ...
            img = self.p_sample(img, conditioning_features, depth_features, extra_cond, times, times_next)
