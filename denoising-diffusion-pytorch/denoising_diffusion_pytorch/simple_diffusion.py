...
# diffusion helpers 补充t维度与x相同（补1）
...
def logsnr_schedule_cosine(t, logsnr_min = -15, logsnr_max = 15):
    ...
    return -2 * log(torch.tan(t_min + t * (t_max - t_min))) #返回的是t处的logsnr，t为归一化的0-1之间的值
...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        *,
        image_size,
        channels = 3,
        pred_objective = 'v',  # progressive distillation 提出的目标,不管它
    ...
    def p_mean_variance(self, x, time, time_next):
        ...
        c = -expm1(log_snr - log_snr_next) # 时间步之间的平滑因子，表示时间步间的噪声变化。expm1(x) 是计算 exp(x) - 1 的函数。

        squared_alpha, squared_alpha_next = log_snr.sigmoid(), log_snr_next.sigmoid() # squared_alpha 是信噪比的 sigmoid，表示信号的能量部分。
        squared_sigma, squared_sigma_next = (-log_snr).sigmoid(), (-log_snr_next).sigmoid() # squared_sigma 是信噪比的反 sigmoid，表示噪声的能量部分。

        alpha, sigma, alpha_next = map(sqrt, (squared_alpha, squared_sigma, squared_alpha_next)) # 对应的平方根值

        batch_log_snr = repeat(log_snr, ' -> b', b = x.shape[0]) #  将 log_snr 扩展到批量的形状
        pred = self.model(x, batch_log_snr) # UNet传入的是xt与logsnr（t）
    ...
    def p_sample_loop(self, shape):
        ...
        steps = torch.linspace(1., 0., self.num_sample_steps + 1, device = self.device) #  1 到 0 的等间距序列，包含 self.num_sample_steps + 1 个值
        ...
        img.clamp_(-1., 1.) # 确保图像像素值被限制在 -1 到 1 之间
