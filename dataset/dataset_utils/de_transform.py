...
def perturb_seg(gt, iou_target=0.6):
    ...
    for _ in range(250):
        for _ in range(4):
            lx, ly = np.random.randint(w), np.random.randint(h) # 矩形的左上角为 (lx, ly)，右下角为 (lw, lh)
            ...
            if np.random.rand() < 0.25:
                cx = int((lx + lw) / 2)
                cy = int((ly + lh) / 2)
                seg[cy, cx] = np.random.randint(2) * 255 # 以 25% 的概率随机选择矩形区域的中心像素 (cx, cy)，并将其随机设置为 0 或 255。

            if np.random.rand() < 0.5:
                seg[ly:lh, lx:lw] = random_dilate(seg[ly:lh, lx:lw])
            else:
                seg[ly:lh, lx:lw] = random_erode(seg[ly:lh, lx:lw])
        # IoU 值小于目标值 iou_target，则提前退出循环，表示当前的扰动达到了预期效果。
