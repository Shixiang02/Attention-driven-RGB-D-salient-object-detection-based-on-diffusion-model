...
class PolypObjDataset(data.Dataset):
    def __init__(self, image_root, depth_root, gt_root, trainsize, mean=None, std=None, randomPeper=True,
        ...
        ], additional_targets={'depth': 'image', 'mask': 'mask'}, is_check_shapes=False)
    ...
    def __getitem__(self, index):
        ...
        # data augumentation
        image, depth, gt = self.aug_transform(image=np.asarray(image),depth=np.asarray(depth),mask=np.asarray(gt)).values()
        image, depth, gt = albumentations.PadIfNeeded(*image_size[::-1], border_mode=0)(image=image, depth=depth, mask=gt).values()
        image, depth, gt = albumentations.RandomCrop(*image_size[::-1])(image=image, depth=depth, mask=gt).values()
